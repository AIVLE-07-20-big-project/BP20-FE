...
import { SALES_TARGETS } from "../../mocks";
...
function ScoreBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[11px] font-bold tabular-nums w-7 text-right">{value}</span>
    </div>
  );
}
...
export function SalesTargetsPage() {
  ...
  const [pipelineMap, setPipelineMap] = useState<Record<string, PipelineStatus>>(
    Object.fromEntries(SALES_TARGETS.map((t) => [t.id, t.pipelineStatus]))
  );
    ...
    <PageShell title="영업 타겟" subtitle="AI가 추천한 신규 영업 대상 비가맹점입니다." freshness="오늘 09:42 기준">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-3">
          {SALES_TARGETS.map((t) => {
            const pipeline = pipelineMap[t.id];
            const style = PIPELINE_STATUS_STYLE[pipeline];
            return (
              <div
                key={t.id}
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded ${style.bg} ${style.text}`}>{pipeline}</span>
                    </div>
                    <div className="text-xs text-muted-foreground">{t.industry} · {t.region}</div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-2xl font-black text-[#087F65] tabular-nums">{t.score}</div>
                ...
                <div className="flex gap-1 mt-3">
                  {PIPELINE_STAGES.slice(0, 5).map((stage) => (
                    <button
                      key={stage}
                      onClick={(e) => { e.stopPropagation(); setPipelineMap((m) => ({ ...m, [t.id]: stage })); }}
