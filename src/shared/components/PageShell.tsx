...
interface PageShellProps {
  ...
  freshness?: string;
  liveFreshness?: boolean;
}

export function PageShell({ title, subtitle, actions, children, freshness, liveFreshness = false }: PageShellProps) {
  return (
    <div className="flex flex-col h-full min-h-0 print:block print:h-auto">
      <div className="flex items-start justify-between gap-4 px-6 pt-6 pb-4 flex-shrink-0 print:px-0">
        <div>
          ...
          {liveFreshness ? (
            <LiveDateTime className="flex mt-1 print:hidden" />
          ) : freshness ? (
